...
def run_analysis(folder_path):
    ...
    for fname in tsv_files:
        ...
        try:
            ...
            series_data = pd.Series(df.values[:, 1].astype(np.float64))
    ...
    try:
        numeric_vals = merged_df.astype(np.float64)
    ...
    out_csv = os.path.join(folder_path, "merged_clustering_compression_statitic.csv")
    df_results.to_csv(out_csv, index=False)
    print(f"Results saved to: {out_csv}")

if __name__ == "__main__":
    folder_path = r"C:\Users\jamalids\Downloads\dataset\HPC-64"
