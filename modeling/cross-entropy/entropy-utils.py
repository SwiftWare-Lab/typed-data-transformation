...
def calculate_multivariate_mutual_information(datasets):
    ...
    # take each row of stacked_data as a tuple, count exact rows
    unique_rows, counts = np.unique(stacked_data, axis=0, return_counts=True)
    # right after unique_rows, counts
    print("stacked_data.shape[0]:", stacked_data.shape[0])
    print("sum(counts):", counts.sum())
    print("unique_rows[:5]:", unique_rows[:5], "counts[:5]:", counts[:5])

    joint_probabilities = counts / stacked_data.shape[0]
    joint_entropy = calculate_entropy(joint_probabilities)
...
def process_dictionary(dicts_or_stat, interaction_info, joint_entropy_dict, conditional_entropies):
    ...
    stat_dict_df.to_csv("/home/jamalids/Documents/fast.csv")
    ...
    plt.savefig(f"/home/jamalids/Documents/{ds_name} Entropy K2")
...
data_set_name = ""
compression_tool = CompressionTool.SNAPPY
# generate a list of entropies arrays
entropy_array = all_possible_ent(1) #[[2, 2, 2, 2], [6, 2, 2, 3], [2, 6, 2, 6], [2, 2, 6, 2], [5, 1, 2, 6], [4, 4, 4, 4], [7, 3, 4, 1]]
