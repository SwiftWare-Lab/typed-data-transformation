...
####################################
# ────────────────────────────────────────────────────────────────
#  Build and plot a dendrogram from a symmetric distance matrix
# ────────────────────────────────────────────────────────────────
from scipy.cluster.hierarchy import linkage, dendrogram
from scipy.spatial.distance import squareform

def plot_dendrogram_from_matrix(D, labels=None, title="relative-entropy dendrogram",
                                png_name="relative_entropy_dendro.png"):
    """
    D : 2-D numpy array, symmetric, zeros on the diagonal
    labels : list of strings for the leaf names
    Saves a PNG and shows the dendrogram.
    """
    Z = linkage(squareform(D), method="single")   # or "average"
    plt.figure(figsize=(6, 4))
    dendrogram(Z, labels=labels, color_threshold=0)
    plt.title(title); plt.tight_layout()
    plt.savefig(png_name, dpi=300)
    print(f"Dendrogram saved → {png_name}")
...
def compress_with_zstd(data, level=3):
    if compression_tool == CompressionTool.HUFFMAN:
        ...
        from modeling.compression_tools import huffman_compress
...
def get_compression_ratio_stats(components, merging_indices, original_dataset=None):
    ...
    ##############33333333
    # ── end of the double-for that populates relative_entropy & merged_cross_entropy
    eps = 1e-12
    relative_entropy[np.isinf(relative_entropy)] = eps
    merged_cross_entropy[np.isinf(merged_cross_entropy)] = eps

    # take the mean of the upper-triangle (one value per cluster pair)
    rel_entropy_val = relative_entropy[np.triu_indices(len(merged_comp), 1)].mean()
    cross_entropy_val = merged_cross_entropy[np.triu_indices(len(merged_comp), 1)].mean()
    ...
    ################################
    relative_entropy[np.isinf(relative_entropy)] = 1e-12
    # --- NEW: show dendrogram for this set of merged clusters -----
    if len(merged_comp) > 1:                       # need ≥2 clusters
        labels = [f"C{i}" for i in range(len(merged_comp))]
        plot_dendrogram_from_matrix(relative_entropy, labels,
                                    title="Relative-entropy dendrogram",
                                    png_name="rel_entropy_dendro.png")


    return comp_ratio_reo, decomp_cr_all, cr_original, merged_comp_entropy, merged_cross_entropy, relative_entropy, merged_k2_entropy, entropy_k2_reorder,rel_entropy_val,cross_entropy_val
...
def all_possible_merging(comp_array, ds_name, original_dataset=None):
    ...
    for comb in partitions:
        ...
        cr_reordered, decomp_cr, orignal_cr, entropy_combined, cross_ent_combined, rel_ent_combined, k2_entropy_combined, k2_entropy_reorder ,el_entropy_val,cross_entropy_val= get_compression_ratio_stats(comp_array, comb, original_dataset)
        ...
        dicts_or_stat_item["el_entropy_val"] = el_entropy_val
        dicts_or_stat_item["cross_entropy_val"] = cross_entropy_val
...
def process_dictionary(dicts_or_stat, interaction_info, joint_entropy_dict, conditional_entropies):
    ...
    stat_dict_df.to_csv("stat_dict.csv", index=False)
