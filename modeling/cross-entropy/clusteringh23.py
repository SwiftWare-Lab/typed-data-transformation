...
def extract_h2(byte_group, num_blocks=256):
    """
    Partition `byte_group` into `num_blocks` equally‐sized, non-overlapping
    windows (dropping any trailing remainder), and compute the 2nd-order entropy H₂
    of each window.

    Returns a 1D numpy array of length `num_blocks`.
    """
    n = len(byte_group)
    # integer window size so that we get exactly num_blocks windows
    window_size = n // num_blocks
    if window_size < 1:
        raise ValueError(f"byte_group too short ({n} samples) to form {num_blocks} blocks")

    h2_vals = []
    for i in range(num_blocks):
        start = i * window_size
        end = start + window_size
        window = byte_group[start:end]
        h2 = compute_kth_entropy(window, k=2)
        h2_vals.append(h2)

    return np.array(h2_vals)

def extract_Sh2(byte_group, window_size= 65536):
    """
    Partition `stream` into non‐overlapping windows of length `window_size`
    (dropping any remainder shorter than window_size), and compute the
    2nd-order entropy H₂ of each window.

    Returns a 1D numpy array of length floor(len(stream)/window_size).
    """
    n = len(byte_group)
    ...
    return np.array([np.mean(h2_vals), np.std(h2_vals), np.max(h2_vals), np.min(h2_vals)])


def extract_STDh2(byte_group, window_size= 65536):
    """
    Partition `stream` into non‐overlapping windows of length `window_size`
    (dropping any remainder shorter than window_size), and compute the
    2nd-order entropy H₂ of each window.
    ...
    return np.array([ np.std(h2_vals)])
...
if __name__ == "__main__":
    folder_path =  '/home/jamalids/Documents/2D/data1/Fcbench/Fcbench-dataset/32/TS'
