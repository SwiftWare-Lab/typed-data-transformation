...
def all_possible_merging(comp_array, original_dataset=None, dataset_name="dataset"):
    bit_width = len(comp_array)
    elements = list(range(bit_width))
    partitions = generate_partitions(elements)

    results = []

    for comb in partitions:
        cr_reordered, decomp_cr, rignal_cr, entropy_combined, cross_ent_combined = get_compression_ratio(
            comp_array, comb, original_dataset)

        results.append({
            "combination": str(comb),
            "reordered_CR": cr_reordered,
            "decomposed_CR": decomp_cr,
            "original_CR": rignal_cr,
            "entropy_combined": entropy_combined,
            "cross_entropy_combined": str(cross_ent_combined)  # Convert matrix to string
        })

    # Compute interaction info (total correlation or V) and joint entropy
    interaction_info, joint_entropy_dict = compute_interaction(comp_array)

    # Add to each row (same value for all rows)
    for row in results:
        row["interaction_info"] = interaction_info
        row["joint_entropy"] = str(joint_entropy_dict)


    # Save to CSV with dataset name
    save_path = f"/home/jamalids/Documents/{dataset_name}_merging_results.csv"
...
if data_set_name == "":
    ...
    float_stream, comp_array, comp_entropy_array = generate_float_stream(
            1*1024, entropies)
    dataset_name="random"
    all_possible_merging(comp_array, float_stream, dataset_name)

else:
    folder_path = "/home/jamalids/Documents/2D/data1/Fcbench/Fcbench-dataset/32/DB"
    ...
    for tsv_file in tsv_files:
        ...
        folder_path1 = "/home/jamalids/Documents/"  # Update as needed
        ...
        # Total float32 values needed
        total_values_needed = 256

        # Number of columns
        num_cols = len(byte_columns)

        # Compute number of rows needed
        num_rows = total_values_needed // num_cols

        # Slice only required portion (first num_rows rows)
        ts_1kb = ts_data.loc[:num_rows - 1, byte_columns]

        byte_data = ts_data.loc[:num_rows - 1, byte_columns].to_numpy().astype(np.float32)
        byte_data = byte_data.flatten(order='F').tobytes()
        component_sizes = [1, 1, 1, 1]
        comp_array = split_bytes_into_components(byte_data, component_sizes)
        float_stream=byte_data
        float_stream = np.frombuffer(float_stream, dtype=np.uint8)

        comp_array = [np.ascontiguousarray(comp.astype(np.uint8)) for comp in comp_array]
        all_possible_merging(comp_array, float_stream, dataset_name)
    ...
    pass
#print("comp_entropy_array", comp_entropy_array)
