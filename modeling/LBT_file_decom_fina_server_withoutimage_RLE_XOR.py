...
def decomposition_based_compression(image_ts, leading_zero_pos, tail_zero_pos, m, n):
    ...
    return (lead_comp_size, tail_comp_size, content_comp_size, lead_comp_size_d, tail_comp_size_d, content_comp_size_d,lead_entropy, tail_entropy, content_entropy,lead_shape_m,
            ...
            content_array_orig1,trailing_mixed_array_orig1,leading_RlE,content_RlE,tailing_RlE)
...
def run_and_collect_data(dataset_path):
   ...
   # datasets = [os.path.join(dp, f) for dp, dn, filenames in os.walk(dataset_path) for f in filenames if f.endswith('.tsv')]
    #dataset_path = "/media/samira/sa/result-compression/data1/citytemp_f32.tsv"
   ...
   # plt.subplots_adjust(hspace=1)  # Adjust the space between rows
    ...
    # Zstd and Huffman
    zstd_compressed_ts, comp_ratio_zstd_default = compress_with_zstd(group_zstd)
    zstd_compressed_ts_l22, comp_ratio_l22 = compress_with_zstd(group_zstd, 22)
    ...
    #tttt=xor_ieee754_consecutive(bool_array3)
    ...
    compressed_size_w, encoded_size_w, dic_size_bits_w = measure_total_compressed_size(encoded_text, inverse_cw_dict)
    ...
    for m in pattern_size_list:
        for n in n_list:
            ...
            bool_array = float_to_ieee754(group)
            #bool_array=xor_ieee754_consecutive(bool_array1)
           ...
           # tot_compressed_size, tot_encoded_size, tot_dic_size_bits = measure_total_compressed_size(
            ...
            (encoded_text_leading, encoded_text_trailing, encoded_text_content, dict_leading, dict_trailing,
             ...
             trailing_mixed_array_orig,leading_RlE,content_RlE,tailing_RlE) = decomposition_based_compression(bool_array, l_z_array, t_z_array, m, n,
                                                                          )
            ##############################################decode#######################
