        ...
        for n in n_list:
           ...
           # tot_compressed_size, tot_encoded_size, tot_dic_size_bits = measure_total_compressed_size(
            ...
            (encoded_text_leading, encoded_text_trailing, encoded_text_content, dict_leading, dict_trailing,
             ...
             trailing_mixed_array_orig,leading_RlE,content_RlE,tailing_RlE,leading_zstd,  content_zstd,tailing_zstd,
            leading_zstd_22,content_zstd_22, tailing_zstd_22,leading_zstd_R, content_zstd_R,tailing_zstd_R,
            leading_zstd_22_R,content_zstd_22_R, tailing_zstd_22_R) = decomposition_based_compression(bool_array, l_z_array, t_z_array, m, n)
...
################################################################
            ...
            for idx in zstd_encoded_b_22:
                result_row[f"zstd_22_com_ratio_b{idx}"] = bool_array_size_bits / (zstd_encoded_b_22[idx] + size_metadata) if \
                    zstd_encoded_b_22[idx] > 0 else None
            for idx in zstd_encoded_b:
                result_row[f"zstd_com_ratio_b{idx}"] = bool_array_size_bits / (zstd_encoded_b[idx] + size_metadata) if \
