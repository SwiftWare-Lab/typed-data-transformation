...
def run_analysis(folder_path):
    ...
    tsv_files = [f for f in os.listdir(folder_path) if f.lower().endswith('.csv')]
    if not tsv_files:
    ...
    for fname in tsv_files:
        ...
        numeric_vals = df.values[1:, :].astype(np.int32)
        print("Number of columns:", numeric_vals .shape[1])
    ...
    out_csv = os.path.join("/home/jamalids/Documents", "clustering_compression_results32.csv")
    df_results.to_csv(out_csv, index=False)
    print(f"Results saved to: {out_csv}")

if __name__ == "__main__":
    folder_path ="/home/jamalids/Documents/dataset"
