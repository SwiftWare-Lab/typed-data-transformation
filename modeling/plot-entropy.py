...
# file_path = "/home/jamalids/Documents/merged128.csv"
...
#
# plt.tight_layout()
# plt.savefig("/home/jamalids/Documents/entropy-row1.png")
...
# Create a unique dataset identifier by combining name + decomposition
df["dataset_identifier"] = df["dataset_name"] + " (" + df["decomposition"] + ")"

# Group by dataset and take the average if there are multiple rows per dataset
df_grouped = df.groupby("dataset_identifier").mean().reset_index()

# Extract dataset names after grouping
dataset_names = df_grouped["dataset_identifier"]
...
for i in range(len(df_grouped)):
    avg_orig = np.array([df_grouped.loc[i, f"orig_k{k}"] for k in range(num_k)])
    avg_tdt = np.array([df_grouped.loc[i, f"tdt_k{k}"] for k in range(num_k)])
    avg_custom = np.array([df_grouped.loc[i, f"custom_k{k}"] for k in range(num_k)])
    avg_after = np.array([df_grouped.loc[i, f"tdt_after_custom_k{k}"] for k in range(num_k)])
...
ax1.set_xlabel("Dataset Name (with decomposition)")
ax1.set_ylabel("Highest Ratio (Log Scale)")
ax1.set_title("Maximum Ratio Comparison for Each Dataset (Averaged Over Multiple Rows)")
ax1.set_xticks(x)
ax1.set_xticklabels(dataset_names, rotation=90, fontsize=9)
ax1.set_yscale("log")  # Set log scale on y-axis
ax1.grid(axis='y')
...
# Second y-axis for Zstd ratios (Log Scale)
ax2 = ax1.twinx()
zstd_ratios = df_grouped[["standard zstd ratio", "decomposed zstd row-order ratio", "reordered zstd row-order ratio"]].values
...
plt.tight_layout()
plt.savefig("/home/jamalids/Documents/entropy-averaged.png")
print("Saved plot to /home/jamalids/Documents/entropy-averaged.png")
