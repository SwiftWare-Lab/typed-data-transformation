...
def generate_byte_stream(size, entropy):
    ...
    if entropy <= 8:
        symbols = np.ones(num_symbols, dtype=np.uint8)
    elif entropy <= 16:
        symbols = np.ones(num_symbols, dtype=np.uint16)
    elif entropy <= 32:
        symbols = np.ones(num_symbols, dtype=np.uint32)
    elif entropy <= 64:
        symbols = np.ones(num_symbols, dtype=np.uint64)
    else:
...
for i in range(1, 8):
    float_stream, comp_array, comp_entropy_array = generate_float_stream(2*1024*1024, [4, 4, i, 8-i])
