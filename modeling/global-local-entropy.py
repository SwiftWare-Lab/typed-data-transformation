...
def compute_huffman_code(dict_code,original_data_bool,m, n):
    if not dict_code:
        print("Warning: dict_code is empty. Returning default values.")
        # Return default values for estimated_size, total_size, and other outputs
        return 0, 0, {}, None, 0, ""
...
def decomposition_based_compression(image_ts, leading_zero_pos, tail_zero_pos, m, n):
    ...
    if leading_zero_pos.size == 0:
        print("Warning: leading_zero_pos is empty. Returning default values.")
        min_lead, max_lead, avg_lead = 0, 0, 0  # Default values or handle appropriately
    else:
    ...
    if tail_zero_pos.size == 0:
        print("Warning: tail_zero_pos is empty. Returning default values.")
        min_tail, max_tail, avg_tail = 0, 0, 0  # Default values or handle appropriately
    else:
        min_tail = int(np.min(tail_zero_pos))
        max_tail = int(np.max(tail_zero_pos))
        avg_tail = int(np.mean(tail_zero_pos))
...
def huffman_code_array(array):
    ...
    # If the frequency dictionary is empty, return 0 or handle the case gracefully
    if not frq_dict:
        print("Error: The frequency dictionary is empty. Cannot create Huffman codes.")
        return 0, 0  # Returning 0 as fallback, you can adjust this based on your use case
   ...
   # datasets = [dataset_path]
    ...
    for dataset_path in datasets:
        ...
        for block_idx in range(num_blocks):
            ...
            print(f"Processing block {block_idx + 1}/{num_blocks} for dataset {dataset_name}")

            # Calculate the total number of elements
            total_elements = len(block_data)
            positive_values = np.sum(block_data > 0)
            negative_values = np.sum(block_data < 0)

            # Zstd
            zstd_compressed_ts, comp_ratio_zstd_default = compress_with_zstd(block_data)
            zstd_compressed_ts_l22, comp_ratio_l22 = compress_with_zstd(block_data, 22)
           ...
           # non_consecutive_array=block_data
            ...
            inverse_cw_dict, root, tree_size, encoded_text = pattern_based_compressor(bool_array3, m1, n1, ts_m1, 32)
            ...
            # Begin pattern size and decomposition loop after processing the block
            pattern_size_list = [8]
            n_list = [1]
            for m in pattern_size_list:
                for n in n_list:
                    ...
                    (encoded_text_leading, encoded_text_trailing, encoded_text_content, dict_leading, dict_trailing,
                     ...
                     tailing_zstd,
                     leading_zstd_22, content_zstd_22, tailing_zstd_22, leading_zstd_R, content_zstd_R, tailing_zstd_R,
                     leading_zstd_22_R, content_zstd_22_R, tailing_zstd_22_R, leading_gzip_R, content_gzip_R,
                     tailing_gzip_R,
                     leading_gzip, content_gzip, tailing_gzip) = decomposition_based_compression(bool_array, l_z_array,
                                                                                                 t_z_array, m, n)
                    ...
                    for idx, (
                    encoded_array, dictionary, content_entropy1, content_shap_m1, content_shap_n1) in enumerate(
                            zip(encoded_text_content, dict_content, content_entropy, content_shap_m, content_shap_n),
                            start=1):
                        ...
                        result_row[f"b{idx}_content_Wieght_size"] = (
                                                                                content_shap_m1 * content_shap_n1) / bool_array_size_bits
