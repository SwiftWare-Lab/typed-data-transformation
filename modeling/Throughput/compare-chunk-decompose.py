...
import re
###################
directories = ['/home/jamalids/Documents/results-lz4']
dataframes = []
...
median_df['RunType'] = median_df['RunType'].replace({

    'Chunk-decompose_Parallel': 'Chunked_Decompose_Parallel',
    'Decompose_Chunk_Parallel':'Decompose_Chunk_Parallel'
})
...
for (dataset, threads, blockSize), group in median_df.groupby(['DatasetName', 'Threads', 'BlockSize']):
    chunked_rows = group[group['ConfigString'] == '{ [1 2]- [3]- [4] }']


    # --- 1) Select the best Chunked_Decompose_Parallel row, if present ---
    chunked_rows = group[group['RunType'] == 'Chunked_Decompose_Parallel']
...
chunked_df = selected_df[selected_df['RunType'].isin(['Chunked_Decompose_Parallel', 'Decompose_Chunk_Parallel'])]
L2_value =409600
chunked_df = chunked_df[chunked_df['BlockSize'] == L2_value].copy()

if not chunked_df.empty:
    for run_type in ['Chunked_Decompose_Parallel', 'Decompose_Chunk_Parallel']:
        ...
        for idx, row in best_rows.iterrows():
            ...
            if dataset not in full_rows_added:
                ...
                if run_type == 'Chunked_Decompose_Parallel':
