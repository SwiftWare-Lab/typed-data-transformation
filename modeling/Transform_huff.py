...
def replace_patterns_with_cw(bool_array, rectangles, m, n,ts_m, ts_n):
...
def pattern_based_compressor(original_data_bool, m, n,ts_m, ts_n):
    ...
    compressed_char_array = replace_patterns_with_cw(original_data_bool, cw_dict, m, n,ts_m, ts_n)
    # convert to bytes
...
def run_and_collect_data(dataset_path):
    ...
    for group_id, group in groups:
        ...
        #chunks = [group[i:i + chunk_size].to_numpy().reshape(-1) for i in range(0, len(group), chunk_size)]
        #group1=chunks
        ...
        #save dictionary
        with open('hst_wfc3_uvis_f32.pkl', 'wb') as pickle_file:
...
if __name__ == "__main__":
    ...
    df_results.to_csv('hst_wfc3_uvis_f32.csv')
   # df_results.to_csv(log_file, index=False, header=True)