  ...
  int iter=20;
    ...
    for (const auto& componentSizes : componentSizesList) {
      ...
      for (int i = 0; i < iter; ++i) {
        ...
        double compressedSize = snappyCompression(globalByteArray, pi_full, compressedData);
       ...
       snappyDecompression(compressedData, decompressedData, pi_full);
        ...
        start = std::chrono::high_resolution_clock::now();
        compressedSize = snappyDecomposedSequential(globalByteArray, pi_seq, compressedComponents, componentSizes);
        end = std::chrono::high_resolution_clock::now();
        pi_seq.total_time_compressed = std::chrono::duration<double>(end - start).count();

        start = std::chrono::high_resolution_clock::now();
        snappyDecomposedSequentialDecompression(compressedComponents, pi_seq, componentSizes);
        end = std::chrono::high_resolution_clock::now();
        ...
        start = std::chrono::high_resolution_clock::now();
        compressedSize = snappyDecomposedParallel(globalByteArray, pi_parallel, compressedComponents, componentSizes, numThreads);
        end = std::chrono::high_resolution_clock::now();
        pi_parallel.total_time_compressed = std::chrono::duration<double>(end - start).count();

        start = std::chrono::high_resolution_clock::now();
        std::vector<uint8_t> decompressedData2 = snappyDecomposedParallelDecompression(compressedComponents, pi_parallel, componentSizes, numThreads);
        end = std::chrono::high_resolution_clock::now();
