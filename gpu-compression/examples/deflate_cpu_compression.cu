...
static void run_example(const std::vector<std::vector<char>>& data, int algo, int level)
{
  ...
  BatchDataCPU compress_data_cpu(
      chunk_size*1.1, input_data_cpu.size());
  ...
  for (size_t i = 0; i < input_data_cpu.size(); ++i) {
    ...
    if(algo==0){ //libdeflate
      ...
      std::cout << "Compressing chunk " << i << " with input size: " << input_data_cpu.sizes()[i]
                << " and output buffer size: " << compress_data_cpu.sizes()[i] << std::endl;
...
std::vector<T> aligned_vector(size_t count) {
  if (count == 0) {
    return std::vector<T>(); // Handle empty case
  }
  size_t bytes = count * sizeof(T);
  void* aligned_memory = aligned_alloc(alignof(T), bytes);
  if (!aligned_memory) {
    throw std::bad_alloc(); // Handle allocation failure
  }
  try {
    std::vector<T> vec(static_cast<T*>(aligned_memory), static_cast<T*>(aligned_memory) + count);
    return vec;
  } catch (...) {
    std::free(aligned_memory); // Important: Free on exception
    throw;
  }
}
...
std::vector<std::vector<char>>
read_and_cluster_no_chunk(const std::string& filename, const std::vector<int>& cluster_sizes){
  // TODO read the file
...
int main(int argc, char* argv[])
{
  ...
  // TODO replace this multifile reading with clustering
  //auto data = multi_file(file_names);
  //auto data = read_and_cluster_chunk(file_names[0], {2, 1, 1});
  auto data = read_and_cluster_no_chunk(file_names[0], {2, 1, 1});
