...
class CompressorConfig:
    def __init__(self, dataset_path: str, m: int, n: int, chunk_size: int, log_file: str):
...
def arg_parser():
    ...
    parser.add_argument('--pattern', dest='pattern', default="10*16", help='Pattern dimensions as m*n.')
    parser.add_argument('--outcsv', dest='log_file', default="./log_out.csv", help='Output CSV file for the results.')
    parser.add_argument('--chunk_size', dest='chunk_size', default=10, type=int, help='Chunk size for processing the dataset.')
    parser.add_argument('--m', dest='m', default=10, type=int, help='Height of the pattern block.')
    parser.add_argument('--n', dest='n', default=16, type=int, help='Width of the pattern block.')
    return parser
...
if __name__ == "__main__":
    ...
    compression_results = read_and_describe_dataset(config)
        

    results_df = pd.DataFrame([compression_results])
    results_df.to_csv(config.log_file, mode='a', index=False, header=not os.path.exists(config.log_file))
