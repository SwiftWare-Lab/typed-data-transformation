...
if __name__ == "__main__":
    ...
    mode = 'signal'
    if mode == 'signal':
        # Predefined groups of block sizes for each n value
        groups = {
            8: [(m, 8) for m in range(1, 11)],
            16: [(m, 16) for m in range(1, 11)],
            32: [(m, 32) for m in range(1, 11)]
        }

        # Process each group sequentially
        for n, block_sizes in groups.items():
            ...
            results_df = pd.DataFrame({
                ...
                'm': [result[3]['stats']['m'] for result in results],  # Get m from stats
                'n': [result[3]['stats']['n'] for result in results],  # Get n from stats
                'Total Occurrences': [result[3]['stats']['total_occurrences'] for result in results],
                'num_patterns': [result[3]['stats']['num_patterns'] for result in results],
                'Size Per Pattern Bit Roundup': [result[3]['stats']['size_per_pattern_bit_roundup'] for result in results],
                'Size Per Pattern Byte Roundup': [result[3]['stats']['size_per_pattern_byte_roundup'] for result in results],
                'Size Uniform Code': [result[3]['stats']['size_uniform_code'] for result in results],
                'entropy': [result[3]['stats']['entropy'] for result in results]
            })

            # Write results to CSV file
            results_df.to_csv(f"{log_file.replace('.csv', '')}_{n}.csv", index=False, header=True)

        # Process other tools after all compressions
        results_other = read_and_describe_dataset_othertools(dataset_path)
        results_other.to_csv(f"{log_file.replace('.csv', '')}_other_tools.csv", index=False, header=True)
