...
if len(sys.argv) != 2:
    print("Usage: python RandomizedSearchCV.py <dataset_name> ")
    sys.exit(1)

# Load the dataset from the command-line argument
dataset_path = sys.argv[1]
data_real= pd.read_csv(dataset_path)  # Corrected to load the dataset into a DataFrame
...
for model_name, train_func, params in models:
    ...
    if feature_importances is not None:
        ...
        out_plot_feature = os.path.join('feature_importance_plots', f'{model_name}_feature_importance_Rand.png')
    ...
    if feature_importances is not None:
         ...
         if isinstance(best_estimator, DecisionTreeClassifier):
            ...
            out_plot_feature = os.path.join('plot', f'{model_name}_feature_importance_rand.png')
            # Save the feature importance plot
            fig.savefig(out_plot_feature)
            # release resources
            plt.close(fig)  
...
out_plot = os.path.join('plot', 'confusion_matrix_plot_Rand.png')
