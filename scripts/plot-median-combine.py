...
directories = ['/media/samira/sa/result-compression/logzstd/log32-L']
...
if dataframes:
    ...
    all_data_output_path = '/media/samira/sa/result-compression/logzstd/combined_all_data_32L.csv'
    ...
    median_data_output_path = '/media/samira/sa/result-compression/logzstd/combined_median_rows_32L.csv'
    ...
    try:
        ...
        for dataset, group in grouped:
            ...
            if not parallel_rows.empty:
                max_parallel_row = parallel_rows.loc[parallel_rows['CompressionRatio'].idxmax()]
                max_comp_ratio_rows.append(max_parallel_row)

                # Find the corresponding Full row for the same dataset
                full_rows = group[group['RunType'] == 'Full']
                if not full_rows.empty:
                    corresponding_full_row = full_rows.iloc[0]  # Select the first Full row
        ...
        max_comp_ratio_output_path = '/media/samira/sa/result-compression/logzstd/max_comp_ratio_parallel_full_32L.csv'
