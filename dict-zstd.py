...
def compress_patterns(patterns, level=10):
    """
    Compress a list of patterns using Zstandard.

    :param patterns: List of patterns where each pattern is a numpy array.
    :param level: Compression level for Zstandard.
...
def decompress_patterns(compressed_data, original_patterns):
    """
    Decompress patterns compressed by Zstandard and verify them against the original patterns.

    :param compressed_data: Compressed byte data.
    :param original_patterns: Original list of patterns, each as a numpy array.
    :return: Tuple (boolean, decompressed_patterns), where boolean indicates match with original.
    """
    ...
    decompressed_bit_string = bin(int.from_bytes(decompressed_data, byteorder='big'))[2:]
    
    # Calculate the expected length of the bit string from original patterns
    original_bit_string = ''.join(map(str, np.concatenate([np.ravel(pattern) for pattern in original_patterns]).astype(int)))
    padding_length = (8 - len(original_bit_string) % 8) % 8
    original_bit_string = original_bit_string + '0' * padding_length

    # Compare decompressed bit string with the original, adjust for padding
    is_matched = decompressed_bit_string.zfill(len(original_bit_string)) == original_bit_string

    # Convert decompressed bit string back to patterns
    bit_array = np.array(list(map(int, decompressed_bit_string[:len(original_bit_string)])))
    decompressed_patterns = []
    start = 0
    for pattern in original_patterns:
        size = pattern.size
        decompressed_pattern = bit_array[start:start + size].reshape(pattern.shape)
...
def process_feature(dataset_name, feature_idx, feature_data3,m,n):
    ...
    print(pattern_list)
    print("pattern_list",pattern_list.size)
