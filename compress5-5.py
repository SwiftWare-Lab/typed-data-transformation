    ...
    for i in range(0, img_shape[0], m):
        for j in range(0, img_shape[1], n):
            ...
            if block_index < len(binary_representations):
                ...
                # Place the pattern in the reconstructed image
...
def process_feature(dataset_name, feature_idx, feature_data2 ,feature_data3):
    # Convert float data to binary image
    # Apply padding to the data to ensure it is divisible by chunk_size
    if feature_data3.ndim != 2:
        raise ValueError("feature_data must be a 2-dimensional array",feature_data.ndim)
...
datasets =[{'name': 'ArticularyWordRecognition', 'type': 'UCR_UEA'},
 {'name': 'AtrialFibrillation', 'type': 'UCR_UEA'},
 {'name': 'BasicMotions', 'type': 'UCR_UEA'},
 {'name': 'CharacterTrajectories', 'type': 'UCR_UEA'},
 {'name': 'FingerMovements', 'type': 'UCR_UEA'},
 {'name': 'LSST', 'type': 'UCR_UEA'}
          ] 
